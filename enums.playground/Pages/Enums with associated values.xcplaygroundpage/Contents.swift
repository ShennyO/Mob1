...
var soccerBall = Thing(name: "SoccerBall")
var soccerBox = Box<Thing>.contains(soccerBall)
var phone = Thing(name: "phone")
var phoneBox = Box<Thing>.contains(phone)
var chocolate = Thing(name: "Chocolate")
var chocoBox = Box<Thing>.contains(chocolate)

func getBoxDetails(box: Box<Thing>) -> String {
      
      switch box {
      case .empty:
            return "This box is empty"
      case .contains(let thing):
            return "This box contains \(thing.name)"
      default:
            return "Error"
      }
